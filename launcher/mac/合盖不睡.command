...
show_state() {
    ...
    explain "自动放开：用电池且电量低于 ${LID_BATTERY_FLOOR}%、低电量模式、合盖后 ${LID_IDLE_HOURS} 小时没有请求。"
...
explain "· 电量低于 ${LID_BATTERY_FLOOR}%、开了低电量模式、合盖 ${LID_IDLE_HOURS} 小时没人用时自动放开，Mac 会睡，并通知手机。"
...
if sudo /usr/sbin/visudo -cf "$tmp" >/dev/null && sudo /usr/bin/install -m 0440 -o root -g wheel "$tmp" "$LID_SUDOERS"; then
    ...
    if [[ -s "$LAN_KEY_FILE" ]]; then
        watchdog_running || watchdog_start
