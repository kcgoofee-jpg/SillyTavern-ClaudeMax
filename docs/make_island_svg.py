...
<title>Claude Max 灵动岛：思考中 → 写作中 → 完成（字数 · 用时 · 缓存）→ 体检提示 → 缩回小点</title>
<rect width="{W}" height="{H}" rx="18" fill="#e9e6e1"/>
<rect x="24" y="{H - 44}" width="{W - 48}" height="28" rx="14" fill="#fff" fill-opacity=".85"/>
<text x="44" y="{H - 25}" font-size="14" fill="#9a958d">输入栏</text>
<text x="{CX}" y="40" font-size="15" fill="#6f6a62" text-anchor="middle">Claude Max · 灵动岛</text>
