...
import { createTurnCollector, hasPinnedContext, pinnedContext, replayTurn, sentTextFor } from './turn-capture.js';
...
import { dumpEntries, dumpRequest } from './debug-dump.js';
import { keepReply } from './reply-keeper.js';
...
function buildQueryConfig({ messages: rawMessages, modelInfo, oneMActive, settings, abortController, stream, env, sdk }) {
    ...
    if (settings.useResume && envFlag('CLAUDE_SUBSCRIPTION_USE_RESUME', true)) {
        try {
            ...
            const replay = envFlag('CLAUDE_SUBSCRIPTION_TURN_REPLAY', true)
                ? (text, parent, m) => replayTurn(text, parent, m, { userOnly: pinOn })
                : null;
            const entries = assembleEntries(split.history, meta, modelInfo.baseId, {
                replay,
                pinned: pinOn ? (parent, m) => pinnedContext(pinKey, parent, m) : null,
            });
            ...
            if (entries.length > 0) {
                ...
                const collector = createTurnCollector(typeof split.current?.content === 'string' ? split.current.content : null, settings.captureKey, pinKey);
                ...
                return { prompt, options, path: 'resume', sessionId, shape: split.shape, collector, currentText: typeof split.current?.content === 'string' ? split.current.content : null };
            }
...
export async function handleChatCompletions(req, res) {
    ...
    if (!settings.auxiliary) try {
        ...
        if (settings.loreTail || settings.foldTail) {
            ...
            if (blocks.length || fold.folded || restored.some((m, i) => m !== history[i])) {
                ...
                if (target >= 0 && target !== lastUser) rememberInjected(plain ?? history[rawTarget].content, withLore[target].content);
    ...
    // A reply with a slot is finished and kept even if the app goes away
    // mid-stream (closed, killed, background): it is paid for, and the panel
    // puts it back when the chat is opened again (lib/reply-keeper.js).
    const keepOnClose = !!settings.replySlot && !settings.auxiliary;
        ...
        while (true) {
            ...
            if (settings.dryRun) {
                ...
                if (cfg.collector && cfg.currentText !== null) {
                    cfg.collector.onAppend([{ type: 'user', message: { role: 'user', content: cfg.currentText } }, { type: 'assistant' }]);
                }
