...
import { createTurnCollector, replayTurn } from './turn-capture.js';
    ...
    if (settings.useResume && envFlag('CLAUDE_SUBSCRIPTION_USE_RESUME', true)) {
        try {
            ...
            if (entries.length > 0) {
                ...
                const collector = createTurnCollector(typeof split.current?.content === 'string' ? split.current.content : null);
