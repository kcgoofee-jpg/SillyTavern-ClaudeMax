...
function buildQueryConfig({ messages: rawMessages, modelInfo, oneMActive, settings, abortController, stream, env, sdk }) {
    const messages = settings.systemPlacement === 'hoist' ? rawMessages : inlineLateSystemMessages(rawMessages, { tail: settings.systemPlacement === 'tail' });
...
export async function handleChatCompletions(req, res) {
    ...
    let cacheDiag = null;
    try {
        const placed = settings.systemPlacement === 'hoist' ? messages : inlineLateSystemMessages(messages, { tail: settings.systemPlacement === 'tail' });
        cacheDiag = diagnoseCache(extractSystemText(placed), placed.filter((m) => m?.role !== 'system'));
        console.log(`${PLUGIN_TAG} ${describeDiag(cacheDiag)}`);
        settings.systemSplitAt = cacheDiag?.splitAt ?? null;
        if (settings.debugDump) dumpRequest({ model: requestedModel, settings, raw: messages, placed, cacheDiag });
    } catch (err) {
    ...
    try {
        ...
        recordRequest({
            model: modelInfo.requested, effort: settings.effort ?? null, placement: settings.systemPlacement, path: lastPath, stream: wantStream, startedAt, firstTokenAt, shape, cacheDiag,
