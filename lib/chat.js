...
import { extractVolatileBlocks, injectBlocks, newLoreOnly } from './lore-tail.js';
...
export async function handleChatCompletions(req, res) {
    ...
    try {
        ...
        let sent = placed;
        if (settings.loreTail && cacheDiag?.volatileTags?.length) {
            const { system, blocks } = extractVolatileBlocks(extractSystemText(placed) ?? '', cacheDiag.volatileTags);
            if (blocks.length) {
                const lastUser = history.findLastIndex((m) => m?.role === 'user');
                const earlier = history.slice(0, Math.max(0, lastUser))
                    .filter((m) => m?.role === 'user' && typeof m.content === 'string')
                    .map((m) => sentTextFor(m.content))
                    .filter(Boolean);
                const fresh = newLoreOnly(blocks, earlier);
                sent = [{ role: 'system', content: system }, ...injectBlocks(history, fresh)];
                messages = sent;
                // Next turn this message comes back without the lore: file the
                // capture under that text so it is replayed as sent.
                if (lastUser >= 0 && typeof history[lastUser].content === 'string') settings.captureKey = history[lastUser].content;
    ...
    try {
        ...
        recordRequest({
            model: modelInfo.requested, effort: settings.effort ?? null, placement: settings.systemPlacement, auxiliary: settings.auxiliary, timing, path: lastPath, stream: wantStream, startedAt, firstTokenAt, shape, cacheDiag,
    ...
    recordRequest({
        model: modelInfo.requested, effort: settings.effort ?? null, placement: settings.systemPlacement, auxiliary: settings.auxiliary, timing, path: lastPath, stream: wantStream, startedAt, firstTokenAt, shape, cacheDiag,
        usage, textChars: collectedText.length, reasoningChars: collectedReasoning.length,
        finish: finishReason, clientClosed,
    });
