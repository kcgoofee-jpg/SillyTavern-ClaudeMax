...
const LAST_REMOTE_FILE = join(ROOT, 'data', 'last-remote-done');
let lastRemoteDoneAt = (() => { try { return Number(readFileSync(LAST_REMOTE_FILE, 'utf8')) || 0; } catch { return 0; } })();
...
export function countInFlight(req, res, next) {
    ...
    const end = () => {
        ...
        if (remote && res.statusCode < 400) {
            ...
            try { writeFileSync(LAST_REMOTE_FILE, String(lastRemoteDoneAt)); } catch { /* data/ missing: memory only */ }
        }
    };
    res.on('finish', end);
    res.on('close', end);
...
export async function macStatus() {
    ...
    const flag = (k) => kv[k] === '1';
    const errors = kv.logDir ? tail(join(kv.logDir, 'proxy.log'), 400).filter((l) => l.includes(' ✗ ')).slice(-3) : [];
    return {
