// ──────────────────────────────────────────────
// 灵动岛：one pill above the input bar that morphs between states
// ──────────────────────────────────────────────
//
...
/**
 * createIsland(doc) → { set(state), notice(n), dismiss() }
...
export function createIsland(doc = document) {
    ...
    function place() {
        const form = doc.getElementById('send_form') ?? doc.getElementById('form_sheld');
        const top = form?.getBoundingClientRect?.().top;
        root.style.bottom = top && top > 0 ? `${Math.round(win.innerHeight - top + 10)}px` : '84px';
    }
