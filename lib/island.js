...
export const STICKY_LIVE_MS = 5000;
const isLiveKind = (kind) => kind === 'thinking' || kind === 'writing';

/** How long a notice stays: sticky ones only until a reply starts being generated. */
export function noticeLife(ms, live) {
    return ms || (live ? STICKY_LIVE_MS : 0);
}
 ...
 *
 * A reply being generated wins over a sticky (ms 0) notice: the notice
 * collapses when generation starts, and one raised during generation stays
 * only STICKY_LIVE_MS. drain() hands the pending notices back (the panel was
 * closed: they become toasts).
 */
export function createIsland(doc = document) {
    ...
    function arm() {
        clearTimeout(timer);
        const life = current ? noticeLife(current.ms, isLiveKind(state.kind)) : 0;
        if (life) timer = setTimeout(() => dismiss(false), life);
    }
    ...
    return {
        ...
        set(patch) {
            ...
            state = { ...state, ...patch };
            // Generation starting: a sticky notice steps aside (the dot still shows online / offline).
            if (current && kindChanged && isLiveKind(state.kind) && !current.ms) return dismiss(false);
            if (current) return;
        ...
        notice(n) {
            ...
            if (n.replace) {
                ...
                if (current?.replace === n.replace) { current = item; shownKey = ''; render(); arm(); return item.id; }
            }
            queue.push(item);
            if (!current) next();
            return item.id;
        },
        /** Drop the notices filed under this key (a key is required: keyless notices stay). */
        clear(replaceKey) {
        ...
        /** Notices not yet seen through (current + queued): removed and returned. */
        drain() {
            const out = [...(current ? [current] : []), ...queue.splice(0)];
            if (current) { current = null; clearTimeout(timer); next(); }
            return out;
        },
