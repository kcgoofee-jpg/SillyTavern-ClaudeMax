...
//
// 'inline' mirrors SillyTavern's own Claude converter (convertClaudeMessages):
// leading system messages form the system prompt; every later system
// message becomes a user message in place, merged into the neighboring user
...
export function inlineLateSystemMessages(messages) {
    ...
    if (!rest.some((m) => m?.role === 'system')) return messages;
