...
function loadCredentialsCached() {
    // The keychain read spawns `security`; the file read is cheap and must
    // stay fresh because refresh rewrites it — only cache keychain results.
    if (credentialCache && Date.now() - credentialCache.at < KEYCHAIN_CACHE_TTL_MS) {
        return credentialCache.value;
    }
    const value = loadCredentials();
    credentialCache = value.source === 'keychain' ? { at: Date.now(), value } : null;
    return value;
}
