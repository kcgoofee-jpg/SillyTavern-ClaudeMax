...
/** Paragraph plan from the preset's word_plan (「正文分 8–11 段」), or null. */
export function paragraphRangeFromPreset(preset) {
    for (const p of enabledPrompts(preset)) {
        const m = String(p?.content ?? '').match(/word_plan::[\s\S]*?正文分\s*(\d+)\s*[–—\-~～到至]\s*(\d+)\s*段/);
        if (m) return [Number(m[1]), Number(m[2])];
    }
    return null;
}

/** True when an ENABLED preset entry asks for a <scene_card> after the prose (镜头卡，给生图). */
export function sceneCardFromPreset(preset) {
    return enabledPrompts(preset).some((p) => String(p?.content ?? '').includes('<scene_card_rule>'));
}

/** Prose paragraphs with at least 20 characters. When the body separates
 *  paragraphs with blank lines, a paragraph is a blank-line block (a line of
 *  dialogue inside it doesn't start a new one — Opus 5.5 writes that way);
 *  otherwise each line is a paragraph. */
export function paragraphCount(body) {
    const long = (t) => t.replace(/\s/g, '').length >= 20;
 ...
 */
export function checkReply({ mes, prevMes = null, words = null, banned = DEFAULT_BANNED, leaks = [], statusJump = 40, secondPerson = false, paragraphs = null, sceneCard = false }) {
    const issues = [];
    ...
    if (paragraphs) {
        const [lo, hi] = paragraphs;
        if (paras > hi + 1) add('paragraphs', `正文 ${paras} 段，预设计划 ${lo}–${hi} 段`);
        else if (paras < lo - 1) add('paragraphs', `正文只有 ${paras} 段，预设计划 ${lo}–${hi} 段`);
    }
    if (sceneCard && !/<scene_card>[\s\S]*?<\/scene_card>/.test(mes)) {
        add('scenecard', '没有镜头卡：配图会自己从正文里猜人物和衣着');
    }
    const ui = (mes.match(/<htm1fenge>[\s\S]*?<\/htm1fenge>/g) ?? []).reduce((n, b) => n + b.length, 0);
    if (ui > 2600) add('ui', `界面代码 ${ui} 字，超过规范的 2000 字上限，手机上加载慢`);
