...
// Where the subscription credential lives, in lookup order:
//   1. CLAUDE_CODE_OAUTH_TOKEN (from `claude setup-token`) — access token only
//   2. <config dir>/.credentials.json — Linux/Windows CLI store
//   3. macOS Keychain item "Claude Code-credentials" — the macOS CLI keeps
...
export function loadCredentials({
    ...
    const envToken = String(env.CLAUDE_CODE_OAUTH_TOKEN ?? '').trim();
    if (envToken) {
        return { source: 'env', creds: { claudeAiOauth: { accessToken: envToken } } };
    }
    try {
        return { source: 'file', creds: JSON.parse(readFile(credentialsPath(), 'utf8')) };
    } catch { /* missing or unreadable — try the keychain */ }
    if (platform === 'darwin') {
        try {
            ...
            return { source: 'keychain', creds: JSON.parse(String(raw).trim()) };
        } catch { /* no keychain item */ }
    }
...
export function loadCredentialsCached(load = loadCredentials, now = Date.now()) {
    ...
    // rewrites it — so cache keychain results and "nothing found" (which also
    // asked the keychain), never the file.
    if (credentialCache && now - credentialCache.at < KEYCHAIN_CACHE_TTL_MS) {
        return credentialCache.value;
    }
    const value = load();
    credentialCache = value.source === 'keychain' || value.source === null ? { at: now, value } : null;
    return value;
}
...
async function doRefresh() {
    ...
    try {
        ...
        writeFileSync(tmpPath, JSON.stringify(latest, null, 2), { mode: 0o600 }); // tokens: owner only, like the CLI's own file
...
export async function fetchQuota({ force = false, retried = false } = {}) {
    ...
    if (res.status === 401) {
        // One refresh + one retry: a token the endpoint keeps refusing must
        // not loop (each round rotates the refresh token).
        if (retried) {
            console.warn(`${PLUGIN_TAG} quota endpoint still returns 401 after a token refresh`);
            return null;
        }
...
export function credentialSummary() {
    const { source, creds } = loadCredentialsCached();
    const oauth = creds?.claudeAiOauth;
    if (!oauth?.accessToken) {
        return { present: false, source: source ?? (process.env.CLAUDE_CONFIG_DIR ? 'CLAUDE_CONFIG_DIR' : 'home') };
    }
